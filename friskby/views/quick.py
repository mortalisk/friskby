...
class Quick(View):
    ...
    def get(self , request):
        ...
        data25all = RawData.objects.filter( timestamp_data__range=(start_time , end_time)).values( "id", "value", "timestamp_data", "sensor_id").order_by('timestamp_data')
        data10all = RawData.objects.filter( timestamp_data__range=(start_time , end_time)).values( "id", "value", "timestamp_data", "sensor_id").order_by('timestamp_data')
        ...
        for d in device_list:
            ...
            data25query = [x for x in data25all if x["sensor_id"] == pm25sensor.id]
            data10query = [x for x in data10all if x["sensor_id"] == pm10sensor.id]
